...
export async function POST(req: NextRequest) {
  // Abuse control: per-IP cap (the per-email limit alone is trivially bypassed
  // by rotating addresses, and every request fans out to Airtable/WhatsApp/email).
  if (!checkIpRate(`diag:${clientIp(req)}`, 5, 10 * 60 * 1000)) {
    return NextResponse.json({ error: "rate_limited" }, { status: 429 });
  }

  const body = await req.json().catch(() => ({}));
  const fullName = cap(body.fullName);
  const email = cap(body.email);
  const phone = cap(body.phone, 60);
  const company = cap(body.company);
  const assetType = cap(body.assetType, 100);
  const { capacityMW, noData } = body;
  ...
  await sendEmail(
    ...
    `<h2>New 7-Day Leak Test request</h2>
     <table cellpadding="6" style="border-collapse:collapse">
       ...
       <tr><td><b>Asset type</b></td><td>${esc(assetType || "—")}</td></tr>
