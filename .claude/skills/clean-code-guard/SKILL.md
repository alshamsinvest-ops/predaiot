...
> **Install note (predaiot):** This is a self-contained copy of `clean-code-guard`
> from [amElnagdy/guard-skills](https://github.com/amElnagdy/guard-skills). Every
> operative rule is inline below. The deep-dive `references/*.md` files are NOT
> bundled here — read them upstream at
> <https://github.com/amElnagdy/guard-skills/tree/master/skills/clean-code-guard/references>
> when you need the source citation or extended reasoning.
>
> **Project context:** predaiot is a Next.js 16 / TypeScript app (App Router, RSC,
> Tailwind v4, `next-intl`, Firebase, Express API in `server.ts`, Anthropic Claude).
> There is no WordPress/WooCommerce here. Defer to `CLAUDE.md`/`README.md`
> conventions — notably: every economic figure comes from `lib/constants.ts`, never
...
**Review mode** (triggered when the user asks you to review, audit, critique, or rate code): walk the review checklist (upstream `references/review-checklist.md`) against the target file(s) and produce a structured findings report. Do not edit code in review mode unless asked.

This skill does not replace project linters, formatters, type checkers, or test runners. Use the project's own tools (`npm run lint`, `npm run typecheck`) for mechanical verification; use this skill for the judgement layer around code quality and review.
...
The rules are defensible — they come from primary sources (Uncle Bob, Fowler, Hunt & Thomas, McCabe, Metz) and from published 2024–2026 research on LLM code generation (see upstream `references/sources.md`). If the user has a context-specific reason to override (e.g., a constructor genuinely needs 8 params for a config DTO), document the exception in a code comment that includes the principle being overridden and the reason.
